...
open class BufferItem<T, P>(
    ...
    override val formattedString: String get() {
        val colorizedType = when(itemStatus){
            BufferItemStatus.Commited-> { itemStatus.name.colorize(Colour.Green) }
            BufferItemStatus.SameAsRecent->{ itemStatus.name.colorize(Colour.Magenta) }
            BufferItemStatus.Buffered->{ itemStatus.name.colorize(Colour.Yellow) }
            BufferItemStatus.Unresolved ->{ itemStatus.name.colorize(Colour.Red) }
        }
